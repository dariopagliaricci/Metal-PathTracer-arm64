...
bool ImGui_ImplOSX_Init(NSView* view)
{
    ...
    bd->KeyEventResponder = [[KeyEventResponder alloc] initWithFrame:view.bounds];
...
static bool ImGui_ImplOSX_ShouldEmulateLeftFromCtrlClick(NSEvent* event)
{
    ...
    // On macOS a Ctrl+Left click is delivered as a right button event. Treat it as left button
    // so Dear ImGui retains the standard "Ctrl+Click to edit" interaction.
...
IMGUI_IMPL_API bool ImGui_ImplOSX_HandleEvent(NSEvent* event, NSView* view)
{
    ...
    {
        const bool emulateLeft = ImGui_ImplOSX_ShouldEmulateLeftFromCtrlClick(event);
        int button = emulateLeft ? 0 : (int)[event buttonNumber];
    ...
    {
        NSEvent* synthesizedEvent = nil;
        if (ImGui_ImplOSX_ShouldEmulateLeftFromCtrlClick(event))
        {
            synthesizedEvent = [NSEvent mouseEventWithType:NSEventTypeLeftMouseDragged
        ...
        NSEvent* locationSourceEvent = synthesizedEvent ? synthesizedEvent : event;
        if (event.window == nil)
        ...
        io.AddMouseSourceEvent(GetMouseSource(locationSourceEvent));
    ...
    if (event.type == NSEventTypeFlagsChanged)
    {
        ...
        if (key != ImGuiKey_None)
        {
            ...
            NSEventModifierFlags changed_flags = [event modifierFlags];
            io.AddKeyEvent(key, (changed_flags & mask) != 0);
            io.SetKeyEventNativeData(key, key_code, -1); // To support legacy indexing (<1.87 user code)
        }
